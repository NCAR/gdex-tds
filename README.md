...
[![tds_stats](https://img.shields.io/badge/tds__stats-dashboard-blue)](https://data.gdex.ucar.edu/special_projects/tds/tds_usage_dashboard.html)
...
├── templates/
│   └── tds_usage_dashboard.html  # Dashboard page that iframes tds_usage_stats.html (see Usage Stats Dashboard section)
...
| `/usr/local/tomcat/.unidata/cache` (TDM container) | `tdm-unidata-cache` | netCDF-Java gzip/bzip2 decompression scratch dir |
...
#### `tdm-cache-clean-cronjob.yaml`
Runs every **Tuesday at 2:00 AM**, timed with the TDM's weekly aggregation build. Deletes files older than 7 days from `tds-persist/tdm-unidata-cache` — netCDF-Java's default gzip/bzip2 decompression scratch dir for the `gdex-tdm` container, mounted at `/usr/local/tomcat/.unidata/cache`. Prevents the scratch cache from growing unbounded across aggregation runs.
...
07:00 AM  gen_stats_plot.py
            └─ read access_log_stats.txt from Boreas S3 → publish tds_usage_stats.html
                 (see TDS Usage Stats Dashboard below)

Monday    01:01 AM  heapdump-clean-cronjob (k8s CronJob)
            └─ delete *.hprof heap dumps older than 7 days

Tuesday   02:00 AM  tdm-cache-clean-cronjob (k8s CronJob)
            └─ delete tdm-unidata-cache files older than 7 days (timed with TDM's weekly aggregation build)
```

`access_log_stats.txt` written at 4 AM is uploaded to Boreas by the **next day's** 12:30 AM backup, which is why the 7 AM plot generation reads the previous day's backup.

---

## TDS Usage Stats Dashboard

Daily at **7:00 AM**, `src/gen_stats_plot.py` reads `access_log_stats.txt` from `s3://gdex/tds-tomcat-logs/` on Boreas (written by [`log-stats-cronjob`](#log-stats-cronjobyaml), uploaded by the nightly [`pv-s3-backup`](#pv-s3-backupyaml)) and generates an interactive Plotly usage chart.

The generated chart is published to the `tds` special project folder on Boreas:

```
/gdex/data/special_projects/tds/tds_usage_stats.html
```

served publicly at `https://data.gdex.ucar.edu/special_projects/tds/tds_usage_stats.html`.

`templates/tds_usage_dashboard.html` in this repo — also placed in the same `special_projects/tds/` folder on Boreas — wraps that chart in a styled dashboard page by `<iframe>`-ing `tds_usage_stats.html`. This dashboard page is the public-facing usage stats view.
